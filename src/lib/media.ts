...
export class AudioStreamer {
  ...
  public getVolume(): number {
    // Check output analyser first (AI speaking), then input (user speaking)
    const analyser = this.analyser || this.inputAnalyser;
  ...
  async startInput(onData: (base64: string) => void) {
    ...
    // Create input analyser so volume reflects microphone input too
  ...
  initOutput() {
    if (this.outputAudioCtx) return; // already initialized
...
export class ScreenStreamer {
  ...
  async start(onData: (base64: string) => void) {
    // Use minimal constraints to maximize browser compatibility
